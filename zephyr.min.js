 /** Zephyr * @version 0.1.7 min * @see https://github.com/Serrin/ * @license MIT */
...
function ToIntegerOrInfinity(argument){let v=Math.trunc(+argument);return (v!==v||v===0)?0:v;}
...
function ToLength(argument){let v=Math.trunc(+argument);v=((v!==v||v===0)?0:v);return Math.min(Math.max(v,0),Math.pow(2,53)-1);}
...
function ToIndex(argument){let v=Math.trunc(+argument);if(v!==v||v===0){return 0;}if(v<0||v>(Math.pow(2,53)-1)){throw new RangeError("ToIndex(); RangeError: "+v);}return v;}
...
const TypedArraySpeciesCreate=(constructor,argumentList)=>Reflect.construct(constructor,argumentList);
...
/** object header **/
const VERSION="Zephyr v0.1.7 min";
