 /** Zephyr * @version 0.1.3 esm * @see https://github.com/Serrin/ * @license MIT */
"use strict";
/** Polyfills **/
...
function ToNumeric(value){var type=((value===null)?"null":(typeof value));if(type==="bigint"||type==="number"){return value;}if(type==="symbol"){throw new TypeError("Symbol: "+value);}return +value;}
const ToNumber=(argument)=>+argument;
const StringToNumber=(str)=>+str;
function ToIntegerOrInfinity(argument){argument=+(argument);if(argument!==argument||1/argument===Infinity||1/argument===-Infinity){return 0;}if(argument===Infinity||argument===-Infinity){return argument;}return Math.trunc(argument);}
const ToInt32=(argument)=>((argument=Math.min(Math.max(-2147483648,Math.trunc(+argument)),2147483647))===argument)?argument:0;
const ToUint32=(argument)=>((argument=Math.min(Math.max(0,Math.trunc(+argument)),4294967295))===argument)?argument :0;
const ToInt16=(argument)=>((argument=Math.min(Math.max(-32768,Math.trunc(+argument)),32767))===argument)?argument :0;
const ToUint16=(argument)=>((argument=Math.min(Math.max(0,Math.trunc(+argument)),65535))===argument)?argument:0;
const ToInt8=(argument)=>((argument=Math.min(Math.max(-128,Math.trunc(+argument)),127))===argument)?argument:0;
const ToUint8=(argument)=>((argument=Math.min(Math.max(0,Math.trunc(+argument)),255))===argument)?argument:0;
const ToUint8Clamp=(argument)=>((argument=Math.min(Math.max(0,Math.trunc(+argument)),255))===argument)?argument:0;
const ToBigInt=(argument)=>BigInt(argument);
const StringToBigInt=(str)=>BigInt(str);
const ToBigInt64=(argument)=>BigInt(argument);
const ToBigUint64=(argument)=>BigInt(typeof argument==="bigint"?(argument>Math.pow(2,64)-1?Math.pow(2,64)-1:argument<0?0:argument):((argument=Math.min(Math.max(0,Math.trunc(Number(argument))),Math.pow(2,64)-1))===argument)?argument:0);
...
const ToPropertyKey=(argument)=>(typeof argument==="symbol"? argument:String(argument));
function ToLength(argument){let v=+argument;if(1/v===Infinity||1/v===-Infinity||v!==v){v=0;}let len=((v===Infinity||v===-Infinity)?v:Math.trunc(v));if(len<0){return 0;}return Math.min(len,Math.pow(2,53)-1);}
function CanonicalNumericIndexString(argument){if(typeof argument==="number"&&1/argument===-Infinity){return -0;}let n=+argument;if(String(n)===argument){return n;}}
function RequireObjectCoercible(argument){if(argument==null){throw new TypeError(Object.prototype.toString.call(argument)+ " is not coercible to Object.");}return argument;}
...
const IsCallable=(argument)=>((argument!=null&&["object","function"].includes(typeof argument))?(typeof argument.call==="function"):false);
const IsConstructor=(argument)=>(typeof argument==="function"&&typeof argument.prototype==="object");
const IsExtensible=(O)=>Object.isExtensible(O);
const IsIntegralNumber=(argument)=>(typeof argument==="number"&&isFinite(argument)&&Math.trunc(argument)===argument);
...
function LengthOfArrayLike(obj){let v=+obj.length;if(1/v===Infinity||1/v===-Infinity||v!==v){v=0;}let len=((v===Infinity||v===-Infinity)?v:Math.trunc(v));if(len<0){return 0;}return Math.min(len,Math.pow(2,53)-1);}
...
const GetIteratorDirect=(obj)=>({"[[Iterator]]":obj,"[[NextMethod]]":obj.next,"[[Done]]":false});
function GetIteratorFromMethod(obj,method){let iterator=Reflect.apply(method,obj,[]);if(((iterator===null)?"null":(typeof iterator))!=="object"){throw new TypeError();}return {"[[Iterator]]":iterator,"[[NextMethod]]":iterator.next,"[[Done]]":false};}
function GetIterator(obj,kind){async function* createAsyncIterable(syncIterable){for(const item of syncIterable){yield item;}}let iterator;if(kind==="ASYNC"){if(obj[Symbol.asyncIterator]===undefined&&obj[Symbol.iterator]===undefined){throw new TypeError("GetIterator(); Error: "+obj+" is not sync/async iterable");}if(obj[Symbol.asyncIterator]!==undefined){iterator=obj[Symbol.asyncIterator]();}if(obj[Symbol.asyncIterator]===undefined&&obj[Symbol.iterator]!==undefined){iterator=createAsyncIterable(obj);}}else{if(obj[Symbol.iterator]===undefined){throw new TypeError("GetIterator(); Error: "+obj+" is not sync iterable");}iterator=obj[Symbol.iterator]();}if(((iterator===null)?"null":(typeof iterator))!=="object"){throw new TypeError("GetIterator(); Error: iterator");}return {"[[Iterator]]":iterator,"[[NextMethod]]":iterator.next,"[[Done]]":false};}
function GetIteratorFlattenable(obj,primitiveHandling){const Type=(O)=>(O===null?"null":typeof O);if(Type(obj)!=="object"){if(primitiveHandling==="REJECT-PRIMITIVES"){throw new TypeError("GetIteratorFlattenable(); TypeError: "+obj+" is not an object" );}if(primitiveHandling==="ITERATE-STRING-PRIMITIVES"&&typeof obj!=="string"){throw new TypeError( "GetIteratorFlattenable(); TypeError: "+obj+" is not a string");}}if(obj[Symbol.iterator]===undefined){var iterator=obj;}else{var iterator=obj[Symbol.iterator]();}if(Type(iterator)!=="object"){throw new TypeError("GetIteratorFlattenable(); TypeError: "+iterator+" is not an object");}return {"[[Iterator]]":iterator,"[[NextMethod]]":iterator.next,"[[Done]]":false};}
...
/* AsyncIteratorClose */
const CreateIteratorResultObject=(value,done)=>({"value":value,"done":done});
...
function StringCreate(value,prototype){if(typeof value!=="string"){ throw new TypeError("StringCreate();error");}return value.slice();}
...
function TypedArrayByteLength(taRecord){function TypedArrayElementSize(O){if(O instanceof Int8Array){return 1;}if(O instanceof Uint8Array){return 1;}if(O instanceof Uint8ClampedArray){return 1;}if(O instanceof Int16Array){return 2;}if(O instanceof Uint16Array){return 2;}if(O instanceof Int32Array){return 4;}if(O instanceof Uint32Array){return 4;}if(O instanceof BigInt64Array){return 8;}if(O instanceof BigUint64Array){return 8;}if(O instanceof Float16Array){return 2;}if(O instanceof Float32Array){return 4;}if(O instanceof Float64Array){return 8;}}return TypedArrayElementSize(taRecord)* taRecord.length;}
...
function NumberToBigInt(number){if(typeof number!=="number"||!Number.isSafeInteger(number)){throw new RangeError("NumberToBigInt();RangeError: number is not an integer");}return number;}
...
function TypedArrayElementSize(O){if(O instanceof Int8Array){return 1;}if(O instanceof Uint8Array){return 1;}if(O instanceof Uint8ClampedArray){return 1;}if(O instanceof Int16Array){return 2;}if(O instanceof Uint16Array){return 2;}if(O instanceof Int32Array){return 4;}if(O instanceof Uint32Array){return 4;}if(O instanceof BigInt64Array){return 8;}if(O instanceof BigUint64Array){return 8;}if(O instanceof Float16Array){return 2;}if(O instanceof Float32Array){return 4;}if(O instanceof Float64Array){return 8;}}
function TypedArrayElementType(O){if(O instanceof Int8Array){return "INT8";}if(O instanceof Uint8Array){return "UINT8";}if(O instanceof Uint8ClampedArray){return "UINT8CLAMPED";}if(O instanceof Int16Array){return "INT16";}if(O instanceof Uint16Array){return "UINT16";}if(O instanceof Int32Array){return "INT32";}if(O instanceof Uint32Array){return "UINT32";}if(O instanceof BigInt64Array){return "BIGINT64";}if(O instanceof BigUint64Array){return "BIGUINT64";}if(O instanceof Float16Array){return "FLOAT16";}if(O instanceof Float32Array){return "FLOAT32";}if(O instanceof Float64Array){return "FLOAT64";}}
...
/** object header **/
const VERSION="Zephyr v0.1.3 esm";
function noConflict(){window.ES=zephyr.__prevES__;return zephyr;}
const zephyr={/** object header **/ VERSION,noConflict,/** API **/ Type,StringIndexOf,StringLastIndexOf,/* IsAccessorDescriptor,*//* IsDataDescriptor,*//* IsGenericDescriptor,*//* FromPropertyDescriptor,*//* ToPropertyDescriptor,*//* CompletePropertyDescriptor,*/ ToPrimitive,OrdinaryToPrimitive,ToBoolean,ToNumeric,ToNumber,StringToNumber,ToIntegerOrInfinity,ToInt32,ToUint32,ToInt16,ToUint16,ToInt8,ToUint8,ToUint8Clamp,ToBigInt,StringToBigInt,ToBigInt64,ToBigUint64,ToString,ToObject,ToPropertyKey,ToLength,CanonicalNumericIndexString,RequireObjectCoercible,ToIndex,IsArray,IsCallable,IsConstructor,IsExtensible,IsIntegralNumber,IsRegExp,IsPropertyKey,IsStringWellFormedUnicode,SameType,SameValue,SameValueZero,SameValueNonNumber,IsLessThan,IsLooselyEqual,IsStrictlyEqual,MakeBasicObject,Get,GetV,Set,CreateDataProperty,CreateDataPropertyOrThrow,CreateNonEnumerableDataPropertyOrThrow,DefinePropertyOrThrow,DeletePropertyOrThrow,GetMethod,HasProperty,HasOwnProperty,Call,Construct,SetIntegrityLevel,TestIntegrityLevel,CreateArrayFromList,LengthOfArrayLike,CreateListFromArrayLike,Invoke,/* OrdinaryHasInstance,*/ SpeciesConstructor,EnumerableOwnProperties,/* GetFunctionRealm,*/ CopyDataProperties,/* PrivateElementFind,*//* PrivateFieldAdd,*//* PrivateMethodOrAccessorAdd,*//* PrivateGet,*//* PrivateSet,*//* DefineField,*//* InitializeInstanceElements,*//* GroupBy,*//* SetterThatIgnoresPrototypeProperties,*/ GetIteratorDirect,GetIteratorFromMethod, GetIterator, GetIteratorFlattenable, IteratorNext, IteratorComplete,IteratorValue, IteratorStep, IteratorStepValue,/* IteratorClose,*//*ifAbruptCloseIterator,*//* AsyncIteratorClose,*/ CreateIteratorResultObject,CreateListIteratorRecord,IteratorToList,OrdinaryObjectCreate,OrdinaryCreateFromConstructor,GetPrototypeFromConstructor,/* RequireInternalSlot,*/ OrdinaryFunctionCreate,/* AddRestrictedFunctionProperties,*/ MakeConstructor,/* MakeClassConstructor,*//* MakeMethod,*//* DefineMethodProperty,*/ SetFunctionName, SetFunctionLength,/* FunctionDeclarationInstantiation,*//* BuiltinCallOrConstruct,*//* CreateBuiltinFunction,*//* BoundFunctionCreate,*/ ArrayCreate,ArraySpeciesCreate,ArraySetLength,StringCreate,StringGetOwnProperty,CreateUnmappedArgumentsObject,/* CreateMappedArgumentsObject,*//* MakeTypedArrayWithBufferWitnessRecord,*/ TypedArrayCreate,TypedArrayByteLength,TypedArrayLength,/* IsTypedArrayOutOfBounds,*//* IsTypedArrayFixedLength,*/ IsValidIntegerIndex,TypedArrayGetElement,TypedArraySetElement,/* IsArrayBufferViewOutOfBounds,*//* ModuleNamespaceCreate,*/ SetImmutablePrototype,ValidateNonRevokedProxy,ProxyCreate,UTF16EncodeCodePoint,CodePointsToString,UTF16SurrogatePairToCodePoint,CodePointAt,StringToCodePoints,/* ParseText,*//* GetTemplateObject,*/ KeyForSymbol,InstallErrorCause,NumberToBigInt,TrimString,CreateArrayIterator,FindViaPredicate,FlattenIntoArray,/* SortIndexedProperties,*/ CompareArrayElements,TypedArrayElementSize,TypedArrayElementType,AddEntriesFromIterable,CompareTypedArrayElements,CreateMapIterator,CreateSetIterator,CanonicalizeKeyedCollectionKey, CreateAsyncFromSyncIterator,/* AsyncFromSyncIteratorContinuation,*/ CreateHTML};
/* ESM */
export default zephyr;
export {/** object header **/ VERSION,noConflict,/** API **/ Type,StringIndexOf,StringLastIndexOf,/* IsAccessorDescriptor,*//* IsDataDescriptor,*//* IsGenericDescriptor,*//* FromPropertyDescriptor,*//* ToPropertyDescriptor,*//* CompletePropertyDescriptor,*/ ToPrimitive,OrdinaryToPrimitive,ToBoolean,ToNumeric,ToNumber,StringToNumber,ToIntegerOrInfinity,ToInt32,ToUint32,ToInt16,ToUint16,ToInt8,ToUint8,ToUint8Clamp,ToBigInt,StringToBigInt,ToBigInt64,ToBigUint64,ToString,ToObject,ToPropertyKey,ToLength,CanonicalNumericIndexString,RequireObjectCoercible,ToIndex,IsArray,IsCallable,IsConstructor,IsExtensible,IsIntegralNumber,IsRegExp,IsPropertyKey,IsStringWellFormedUnicode,SameType,SameValue,SameValueZero,SameValueNonNumber,IsLessThan,IsLooselyEqual,IsStrictlyEqual,MakeBasicObject,Get,GetV,Set,CreateDataProperty,CreateDataPropertyOrThrow,CreateNonEnumerableDataPropertyOrThrow,DefinePropertyOrThrow,DeletePropertyOrThrow,GetMethod,HasProperty,HasOwnProperty,Call,Construct,SetIntegrityLevel,TestIntegrityLevel,CreateArrayFromList,LengthOfArrayLike,CreateListFromArrayLike,Invoke,/* OrdinaryHasInstance,*/ SpeciesConstructor,EnumerableOwnProperties,/* GetFunctionRealm,*/ CopyDataProperties,/* PrivateElementFind,*//* PrivateFieldAdd,*//* PrivateMethodOrAccessorAdd,*//* PrivateGet,*//* PrivateSet,*//* DefineField,*//* InitializeInstanceElements,*//* GroupBy,*//* SetterThatIgnoresPrototypeProperties,*/ GetIteratorDirect,GetIteratorFromMethod, GetIterator, GetIteratorFlattenable, IteratorNext, IteratorComplete,IteratorValue, IteratorStep, IteratorStepValue,/* IteratorClose,*//*ifAbruptCloseIterator,*//* AsyncIteratorClose,*/ CreateIteratorResultObject,CreateListIteratorRecord,IteratorToList,OrdinaryObjectCreate,OrdinaryCreateFromConstructor,GetPrototypeFromConstructor,/* RequireInternalSlot,*/ OrdinaryFunctionCreate,/* AddRestrictedFunctionProperties,*/ MakeConstructor,/* MakeClassConstructor,*//* MakeMethod,*//* DefineMethodProperty,*/ SetFunctionName, SetFunctionLength,/* FunctionDeclarationInstantiation,*//* BuiltinCallOrConstruct,*//* CreateBuiltinFunction,*//* BoundFunctionCreate,*/ ArrayCreate,ArraySpeciesCreate,ArraySetLength,StringCreate,StringGetOwnProperty,CreateUnmappedArgumentsObject,/* CreateMappedArgumentsObject,*//* MakeTypedArrayWithBufferWitnessRecord,*/ TypedArrayCreate,TypedArrayByteLength,TypedArrayLength,/* IsTypedArrayOutOfBounds,*//* IsTypedArrayFixedLength,*/ IsValidIntegerIndex,TypedArrayGetElement,TypedArraySetElement,/* IsArrayBufferViewOutOfBounds,*//* ModuleNamespaceCreate,*/ SetImmutablePrototype,ValidateNonRevokedProxy,ProxyCreate,UTF16EncodeCodePoint,CodePointsToString,UTF16SurrogatePairToCodePoint,CodePointAt,StringToCodePoints,/* ParseText,*//* GetTemplateObject,*/ KeyForSymbol,InstallErrorCause,NumberToBigInt,TrimString,CreateArrayIterator,FindViaPredicate,FlattenIntoArray,/* SortIndexedProperties,*/ CompareArrayElements,TypedArrayElementSize,TypedArrayElementType,AddEntriesFromIterable,CompareTypedArrayElements,CreateMapIterator,CreateSetIterator,CanonicalizeKeyedCollectionKey, CreateAsyncFromSyncIterator,/* AsyncFromSyncIteratorContinuation,*/ CreateHTML};
